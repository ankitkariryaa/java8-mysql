		...
		<div class="row" style="margin: 5px 20px 15px 15px">
			...
			<div class="pull-right">
				<c:if test="${not empty secondaryLogoFilePath}">
					<a href="${secondaryLogoLink}" target="_blank"><img
						src="file.htm?filename=${secondaryLogoFilePath}" height=50
						style="margin-right: 25px"></a>
				</c:if>

				<c:if test="${not empty primaryLogoFilePath}">
					<a href="${primaryLogoLink}" target="_blank"><img
						src="file.htm?filename=${primaryLogoFilePath}" height=50></a>
				</c:if>
			</div>
		</div>
	</div>
